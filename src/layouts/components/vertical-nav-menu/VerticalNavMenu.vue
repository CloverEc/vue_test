...
<template>
    <div class="parentx verticalNavMenuClass">
        <!-- Mobile shouw nav sidebar button -->
        <div class="showNavBtn" :class="is_mobile && showNavSiderFlag ? 'toMoveRight' : ''" v-if="is_mobile">
            <feather-icon class="collapseBtn" @click="showNavBtn(!showNavSiderFlag)" :icon="showNavSiderFlag ? 'ChevronsLeftIcon' : 'ChevronsRightIcon'" id="btnVNavMenuMinToggler ChevronsLeftIcon" />  
        </div>
        <vs-sidebar class="v-nav-menu items-no-padding" v-model="isVerticalNavMenuActive" ref="verticalNavMenu" default-index="-1"
            :click-not-close="clickNotClose" :reduce-not-rebound="reduceNotRebound" :parent="parent" :hiddenBackground="clickNotClose"
            :reduce="reduce" v-hammer:swipe="onMenuSwipe">
            ...
            <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
                ...
                <div class="header-sidebar flex items-end justify-between" slot="header">
                    ...
                    <div v-if="!is_mobile">
                        ...
                        <template v-else-if="!showCloseButton && !verticalNavMenuItemsMin">
                            <feather-icon id="btnVNavMenuMinToggler" class="mr-0 cursor-pointer" :icon="reduce ? 'CircleIcon' : 'DiscIcon'" svg-classes="stroke-current text-primary"
                ...
                <!-- Menu Items -->
                <component :is="scrollbarTag" ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll"
                    @scroll="psSectionScroll" :key="$vs.rtl">
                    <template v-for="(item, index) in menuItemsUpdated">
                        ...
                        <template v-else-if="!item.header">
                            ...
                            <!-- Nav-Item -->
                            <v-nav-menu-item v-if="!item.submenu" :key="`item-${index}`" :index="index" :to="item.slug !== 'external' ? item.url : null"
                                :href="item.slug === 'external' ? item.url : null" :icon="item.icon" :target="item.target" :isDisabled="item.isDisabled"
                                :slug="item.slug">
    ...
    export default {
        ...
        methods: {
            ...
            setVerticalNavMenuWidth() {
                let is_mobile = parseInt(localStorage.getItem('is_mobile'));
                this.is_mobile = is_mobile;
                this.$store.commit('setIsMobile',is_mobile)
            ...
            showNavBtn(showNvSiderFlag) {
                console.log('show-nav-btn',showNvSiderFlag);
                this.showNavSiderFlag = showNvSiderFlag;
                // if(this.is_mobile && showNvSiderFlag) {
                if(showNvSiderFlag) {
                    this.showNavMenu();
                } else {
                    this.closeNavMenu();
